...
from core.policies import DistributionalCNN, DistributionalDuelingCNN, DistributionalDuelingFC, DistributionalFC
...
class AgentDQN(DQNBase):
    ...
    def _setup_distributional_networks(self, agent_config: AgentConfig):
        ...
        if not self.is_atari:
            if agent_config.dueling:
                ...
                self.online_model = DistributionalFC(
                    self.observation_space.shape[0], self.action_space.n, agent_config.hidden_dim, self.n_atoms
                ).to(self.device)
                self.target_model = DistributionalFC(
                    self.observation_space.shape[0], self.action_space.n, agent_config.hidden_dim, self.n_atoms
                ).to(self.device)
        else:
            if agent_config.dueling:
                ...
                self.online_model = DistributionalCNN(
                    self.observation_space.shape, self.action_space.n, agent_config.hidden_dim, self.n_atoms
                ).to(self.device)
                self.target_model = DistributionalCNN(
                    self.observation_space.shape, self.action_space.n, agent_config.hidden_dim, self.n_atoms
                ).to(self.device)
    ...
    def _project_distribution(self, next_distributions, rewards, dones, ns):
        ...
        # Handle the case where lower == upper (when b is an integer)
    ...
    def learn(self):
        ...
        2. Calculate distribution of Q-values for the (s,a) pairs in the batch using online network
        3. Using online network find the next best action a'
        4. Using target network find distribution of Q(s', a')
