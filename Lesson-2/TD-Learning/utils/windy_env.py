...
class WindyBridgeEnv(gym.Env):
    ...
    Attributes:
        ...
        observation_space (spaces.Discrete): The agent's state as a single integer.
    """
    ...
    def __init__(self, render_mode=None):
        ...
        # Define observation space: agent's state as a single integer
    ...
    def _location_to_state(self, location):
        """Converts a (row, col) location to a single integer state."""
        return location[0] * self.grid_size[1] + location[1]

    def _get_obs(self):
        """Returns the agent's location as a single integer state."""
        ...
        if is_on_bridge and not is_moving_forward:
            if self.np_random.random() < self.wind_prob:
                ...
                self._agent_location = np.clip(
                    self._agent_location + direction, [0, 0], [self.grid_size[0] - 1, self.grid_size[1] - 1]
                )
        else:
            # Normal movement
            self._agent_location = np.clip(
    ...
    def _render_frame(self):
        if self.window is None and self.render_mode == "human":
            ...
            self.window = pygame.display.set_mode(
                (self.window_size, self.window_size * self.grid_size[0] // self.grid_size[1])
            )
