...
class D3QN:
    def __init__ (self, 
        ...
        # Initialize target model with same weights as online model
    ...
    def soft_update_target(self) -> None:
        """Soft update the target model with the online model's weights using tau."""
    ...
    def validate(self, n: int = 10, render: bool = False) -> float:
        rewards = []
        for i in range(n):
        ...
        for e in range(n_episodes):
            ...
            while not terminated and not truncated:
                ...
                if len(self.buffer) > self.batch_size:
                    ...
                    learning_steps += 1
                    episode_loss += loss
                    # Soft update target network after each learning step
                    self.soft_update_target()
            # Decay epsilon
            self.epsilon  = max(self.epsilon_min, self.epsilon * self.epsilon_decay)
            avg_loss = episode_loss / learning_steps if learning_steps > 0 else 0
            print(f"Episode {e}/{n_episodes}, Episode Reward: {episode_reward}, Episode Mean loss: {avg_loss: .4f}, Current Epsilon: {self.epsilon: .3f}")
            if e % self.validation_period == 0:
                ...
                if test_reward > best_test_reward:
                    ...
                    model_save_path = os.path.join(self.name, f"{self.name}_best_checkpoint.pth")
                    checkpoint = {
